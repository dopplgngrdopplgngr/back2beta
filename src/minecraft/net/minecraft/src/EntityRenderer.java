...
//            Entity, Material, KeyBinding, EntityPlayer, 
//            Block, RenderGlobal, Config, EntityPlayerSP, 
//            RenderBlocks, BlockLeaves, ItemRendererHD, RenderManager, 
//            WorldInfo, MouseHelper, ScaledResolution, GuiIngame, 
//            GuiScreen, GuiParticle, WorldProvider, ChunkProviderLoadOrGenerate, 
//            ClippingHelperImpl, Frustrum, RenderEngine, RenderHelper, 
//            EffectRenderer, InventoryPlayer, WorldChunkManager, BiomeGenBase, 
//            EntitySmokeFX, EntityRainFX, Tessellator
...
public class EntityRenderer
{
    ...
    private float getFOVModifier(float f)
    {
        ...
        if(Keyboard.isKeyDown(mc.gameSettings.ofKeyBindZoom.keyCode))
        {
            if(!zoomMode)
            {
                zoomMode = true;
    ...
    public void updateCameraAndRender(float f)
    {
        ...
        if(world != null && world.worldProvider != null && updatedWorldProvider != world.worldProvider)
        {
            updateWorldLightLevels();
            updatedWorldProvider = mc.theWorld.worldProvider;
        }
        ...
        if(world != null)
        {
            ...
            long l1 = l % 24000L;
            if(Config.isTimeDayOnly() && !world.multiplayerWorld)
            {
                if(l1 <= 1000L)
                {
            ...
            if(Config.isTimeNightOnly() && !world.multiplayerWorld)
            {
                if(l1 <= 14000L)
                {
        ...
        if(mc.inGameHasFocus)
        {
            ...
            mc.thePlayer.func_346_d(f3, f4 * (float)i1);
        }
        ...
        if(mc.theWorld != null)
        {
            ...
            {
                renderWorld(f, renderEndNanoTime + (long)(1000000000D / (double)k1));
            }
            if(mc.gameSettings.limitFramerate == 2)
            {
            ...
            if(!mc.gameSettings.hideGUI || mc.currentScreen != null)
            {
                ...
                mc.ingameGUI.renderGameOverlay(f, mc.currentScreen != null, k, j1);
                if(mc.gameSettings.ofFastDebugInfo)
                {
        ...
        } else
        {
            ...
            if(mc.gameSettings.limitFramerate == 2)
            {
                long l3 = ((renderEndNanoTime + (long)(1000000000D / (double)k1)) - System.nanoTime()) / 0xf4240L;
                if(l3 < 0L)
                {
                    l3 += 10L;
                }
        ...
        {
            GL11.glClear(256);
            mc.currentScreen.drawScreen(k, j1, f);
    ...
    public void updateWorldLightLevels()
    {
        ...
        if(mc.theWorld.worldProvider == null)
        {
            return;
        }
        float f = mc.gameSettings.ofBrightness;
        float af[] = mc.theWorld.worldProvider.lightBrightnessTable;
        float f1 = 0.05F;
        if(mc.theWorld.worldProvider != null && mc.theWorld.worldProvider.isNether)
        {
            f1 = 0.1F + f * 0.15F;
        }
        float f2 = 3F * (1.0F - f);
        for(int i = 0; i <= 15; i++)
        {
            float f3 = 1.0F - (float)i / 15F;
            af[i] = ((1.0F - f3) / (f3 * f2 + 1.0F)) * (1.0F - f1) + f1;
        }

        Config.setLightLevels(af);
    }
    ...
    private void setupFog(int i, float f)
    {
        ...
        if(cloudFog)
        {
            ...
            if(mc.gameSettings.anaglyph)
            {
                float f12 = (f1 * 30F + f5 * 59F + f9 * 11F) / 100F;
                float f15 = (f1 * 30F + f5 * 70F) / 100F;
                float f18 = (f1 * 30F + f9 * 70F) / 100F;
            }
        } else
        if(entityliving.isInsideOfMaterial(Material.water))
        {
            ...
            GL11.glFogf(2914 /*GL_FOG_DENSITY*/, f2);
            float f6 = 0.4F;
            float f10 = 0.4F;
            float f13 = 0.9F;
            if(mc.gameSettings.anaglyph)
            {
                float f16 = (f6 * 30F + f10 * 59F + f13 * 11F) / 100F;
                float f19 = (f6 * 30F + f10 * 70F) / 100F;
                float f21 = (f6 * 30F + f13 * 70F) / 100F;
            }
        } else
        if(entityliving.isInsideOfMaterial(Material.lava))
        {
            ...
            if(mc.gameSettings.anaglyph)
            {
                float f14 = (f3 * 30F + f7 * 59F + f11 * 11F) / 100F;
                float f17 = (f3 * 30F + f7 * 70F) / 100F;
                float f20 = (f3 * 30F + f11 * 70F) / 100F;
            }
        } else
        {
            ...
            GL11.glFogf(2915 /*GL_FOG_START*/, farPlaneDistance * f4);
            GL11.glFogf(2916 /*GL_FOG_END*/, farPlaneDistance * f8);
        }
    ...
    private WorldProvider updatedWorldProvider;
