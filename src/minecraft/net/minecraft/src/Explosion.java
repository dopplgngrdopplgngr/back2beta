...
public class Explosion
{
    ...
    public Explosion(World world, Damage damage, double d, double d1, double d2, float f)
    {
        isFlaming = false;
...
label0:
        ...
        for(int k2 = 0; k2 < list.size(); k2++)
        {
            ...
            if(d4 <= 1.0D)
            {
                ...
                entity.attackEntityFrom(dmgsrc, (int)(((d13 * d13 + d13) / 2D) * 8D * (double)explosionSize + 1.0D));
                double d14 = d13;
    ...
    {
        worldObj.playSoundEffect(explosionX, explosionY, explosionZ, "random.old_explode", 4F, (1.0F + (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2F) * 0.7F);
        ArrayList arraylist = new ArrayList();
        arraylist.addAll(destroyedBlockPositions);
