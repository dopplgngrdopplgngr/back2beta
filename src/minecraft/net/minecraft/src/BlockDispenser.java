...
public class BlockDispenser extends BlockContainer
{
    ...
    private void dispenseItem(World world, int i, int j, int k, Random random)
    {
        ...
        if(itemstack == null)
        {
            world.playAuxSFXnull(1001, i, j, k, 0);
        } else
        {
            if(itemstack.itemID == Item.arrow.shiftedIndex)
            {
                ...
                world.playAuxSFXnull(1002, i, j, k, 0);
            } else
            if(itemstack.itemID == Item.egg.shiftedIndex)
            {
                EntityEgg entityegg = new EntityEgg(world, d, d1, d2);
                entityegg.func_6141_a(i1, 0.10000000149011612D, j1, 1.1F, 6F);
            ...
            if(itemstack.itemID == Item.snowball.shiftedIndex)
            {
                ...
                if(itemstack.itemID == B2BItems.stoneBrick.shiftedIndex)
                {
                   EntityStoneBrick entitystonebrick = new EntityStoneBrick(world, d, d1, d2);
                   entitystonebrick.func_6141_a(i1, 0.10000000149011612D, j1, 1.1F, 6F);
                   world.entityJoinedWorld(entitystonebrick);
                   world.playAuxSFXnull(1002, i, j, k, 0);
                } else 
            {
                ...
                world.playAuxSFXnull(1000, i, j, k, 0);
            }
            world.playAuxSFXnull(2000, i, j, k, i1 + 1 + (j1 + 1) * 3);
        }
    }
