...
public class BlockRedstoneRepeater extends Block
{
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        ...
        {
            world.setBlockAndMetadataWithNotify(i, j, k, Block.redstoneRepeaterOff.blockID, l);
        } else
        if(!isRepeaterPowered)
        {
            world.setBlockAndMetadataWithNotify(i, j, k, Block.redstoneRepeaterOn.blockID, l);
            if(!flag)
            {
                int i1 = (l & 0xc) >> 2;
                world.scheduleBlockUpdate(i, j, k, Block.redstoneRepeaterOn.blockID, field_22023_b[i1] * 2);
            }
        }
    }
