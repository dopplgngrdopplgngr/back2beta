...
//            World, RenderManager, RenderBlocks, Block, 
//            Config, BlockLeaves, GameSettings, WorldRenderer, 
//            Entity, MathHelper, EntitySorter, TileEntityRenderer, 
//            ICamera, EntityLiving, TileEntity, WorldProvider, 
//            Vec3D, RenderHelper, RenderEngine, EntityRenderer, 
//            MouseHelper, MovingObjectPosition, EntityPlayer, EnumMovingObjectType, 
    ...
    implements IWorldAccess
{
    ...
    public RenderGlobal(Minecraft minecraft, RenderEngine renderengine)
    {
        ...
        bed = BufferUtils.createIntBuffer(0x10000);
        dummyInt0 = 0;
        ...
        for(int j1 = 0; j1 < renderChunksWide; j1++)
        {
            for(int k1 = 0; k1 < renderChunksTall; k1++)
            {
                for(int l1 = 0; l1 < renderChunksDeep; l1++)
                {
                    ...
                    worldRenderers[i2] = new WorldRenderer(worldObj, tileEntities, j1 * 16, k1 * 16, l1 * 16, 16, glRenderListBase + k);
                    if(occlusionEnabled)
                    {
    ...
    public int sortAndRender(EntityLiving entityliving, int i, double d)
    {
        ...
        if(d7 > 16D)
        {
            ...
            int k = Config.getPreloadedChunks() * 16;
            double d8 = entityliving.posX - prevReposX;
            double d9 = entityliving.posY - prevReposY;
            double d10 = entityliving.posZ - prevReposZ;
            double d11 = d8 * d8 + d9 * d9 + d10 * d10;
            if(d11 > (double)(k * k) + 16D)
            {
        ...
        if(mc.gameSettings.ofSmoothFps && i == 0)
        {
            GL11.glFinish();
        }
        if(mc.gameSettings.ofSmoothInput && i == 0)
        {
            Config.sleep(1L);
        }
        int l = 0;
        ...
        if(occlusionEnabled && mc.gameSettings.advancedOpengl && !mc.gameSettings.anaglyph && i == 0)
        {
            int j1 = 0;
            byte byte1 = 20;
            checkOcclusionQueryResult(j1, byte1, entityliving.posX, entityliving.posY, entityliving.posZ);
            for(int k1 = j1; k1 < byte1; k1++)
            {
                sortedWorldRenderers[k1].isVisible = true;
            }

            l += renderSortedRenderers(j1, byte1, i, d);
            int l1 = byte1;
            int i2 = 0;
            byte byte2 = 30;
            int j2 = renderChunksWide / 2;
            while(l1 < sortedWorldRenderers.length) 
            {
                int byte3 = l1;
                if(i2 < j2)
                {
                ...
                if(l1 > sortedWorldRenderers.length)
                {
                    l1 = sortedWorldRenderers.length;
                }
                ...
                checkOcclusionQueryResult(byte3, l1, entityliving.posX, entityliving.posY, entityliving.posZ);
                GL11.glPushMatrix();
                float f = 0.0F;
                float f1 = 0.0F;
                float f2 = 0.0F;
                for(int k2 = byte3; k2 < l1; k2++)
                {
                    WorldRenderer worldrenderer1 = sortedWorldRenderers[k2];
                    ...
                    {
                        float f3 = Math.abs((float)(worldrenderer1.visibleFromX - entityliving.posX));
                        float f5 = Math.abs((float)(worldrenderer1.visibleFromY - entityliving.posY));
                        float f7 = Math.abs((float)(worldrenderer1.visibleFromZ - entityliving.posZ));
                        float f9 = f3 + f5 + f7;
                        if((double)f9 < 10D + (double)k2 / 1000D)
                        {
                            worldrenderer1.isVisible = true;
                            continue;
                        }
                    ...
                    float f6 = (float)((double)worldrenderer1.posYMinus - d2);
                    float f8 = (float)((double)worldrenderer1.posZMinus - d3);
                    float f10 = f4 - f;
                    float f11 = f6 - f1;
                    float f12 = f8 - f2;
                ...
                l += renderSortedRenderers(byte3, l1, i, d);
            }
        } else
        {
            l += renderSortedRenderers(0, sortedWorldRenderers.length, i, d);
        }
        return l;
    }
    ...
    private void checkOcclusionQueryResult(int i, int j, double d, double d1, double d2)
    {
        for(int k = i; k < j; k++)
        {
            ...
            occlusionResult.clear();
            ARBOcclusionQuery.glGetQueryObjectuARB(worldrenderer.glOcclusionQuery, 34919 /*GL_QUERY_RESULT_AVAILABLE_ARB*/, occlusionResult);
            ...
            occlusionResult.clear();
            ARBOcclusionQuery.glGetQueryObjectuARB(worldrenderer.glOcclusionQuery, 34918 /*GL_QUERY_RESULT_ARB*/, occlusionResult);
    ...
    public void renderSky(float f)
    {
        ...
        if(af != null && Config.isSkyEnabled())
        {
            ...
            float f7 = worldObj.getCelestialAngle(f);
            GL11.glRotatef(f7 > 0.5F ? 180F : 0.0F, 0.0F, 0.0F, 1.0F);
            float f9 = af[0];
            float f11 = af[1];
            float f13 = af[2];
            if(mc.gameSettings.anaglyph)
            {
                float f15 = (f9 * 30F + f11 * 59F + f13 * 11F) / 100F;
                float f17 = (f9 * 30F + f11 * 70F) / 100F;
                float f19 = (f9 * 30F + f13 * 70F) / 100F;
                f9 = f15;
                f11 = f17;
                f13 = f19;
            }
        ...
        GL11.glPushMatrix();
        float f8 = 1.0F - worldObj.func_27162_g(f);
        float f10 = 0.0F;
        float f12 = 0.0F;
        float f14 = 0.0F;
        ...
        tessellator.draw();
        GL11.glDisable(3553 /*GL_TEXTURE_2D*/);
        float f18 = worldObj.getStarBrightness(f) * f8;
        if(f18 > 0.0F)
        {
    ...
    public void renderClouds(float f)
    {
        ...
        f9 += mc.gameSettings.ofCloudsHeight * 25F;
    ...
    public void renderCloudsFancy(float f)
    {
        ...
        f4 += mc.gameSettings.ofCloudsHeight * 25F;
            ...
            for(int i1 = -byte0 + 1; i1 <= byte0; i1++)
            {
                for(int j1 = -byte0 + 1; j1 <= byte0; j1++)
                {
                    ...
                    if(i1 > -1)
                    {
                        ...
                        {
                            tessellator.addVertexWithUV(f19 + (float)k1 + 0.0F, (double)(f4 + 0.0F) + d2, f20 + (float)k, (f17 + (float)k1 + 0.5F) * f13 + f9, (f18 + (float)k) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k1 + 0.0F, (double)(f4 + f3) - d2, f20 + (float)k, (f17 + (float)k1 + 0.5F) * f13 + f9, (f18 + (float)k) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k1 + 0.0F, (double)(f4 + f3) - d2, f20 + 0.0F, (f17 + (float)k1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k1 + 0.0F, (double)(f4 + 0.0F) + d2, f20 + 0.0F, (f17 + (float)k1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
                        }
                    ...
                    if(i1 <= 1)
                    {
                        ...
                        {
                            tessellator.addVertexWithUV((f19 + (float)l1 + 1.0F) - f16, (double)(f4 + 0.0F) + d2, f20 + (float)k, (f17 + (float)l1 + 0.5F) * f13 + f9, (f18 + (float)k) * f13 + f11);
                            tessellator.addVertexWithUV((f19 + (float)l1 + 1.0F) - f16, (double)(f4 + f3) - d2, f20 + (float)k, (f17 + (float)l1 + 0.5F) * f13 + f9, (f18 + (float)k) * f13 + f11);
                            tessellator.addVertexWithUV((f19 + (float)l1 + 1.0F) - f16, (double)(f4 + f3) - d2, f20 + 0.0F, (f17 + (float)l1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
                            tessellator.addVertexWithUV((f19 + (float)l1 + 1.0F) - f16, (double)(f4 + 0.0F) + d2, f20 + 0.0F, (f17 + (float)l1 + 0.5F) * f13 + f9, (f18 + 0.0F) * f13 + f11);
                        }
                    ...
                    if(j1 > -1)
                    {
                        ...
                        {
                            tessellator.addVertexWithUV(f19 + 0.0F, (double)(f4 + f3) - d2, f20 + (float)i2 + 0.0F, (f17 + 0.0F) * f13 + f9, (f18 + (float)i2 + 0.5F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k, (double)(f4 + f3) - d2, f20 + (float)i2 + 0.0F, (f17 + (float)k) * f13 + f9, (f18 + (float)i2 + 0.5F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k, (double)(f4 + 0.0F) + d2, f20 + (float)i2 + 0.0F, (f17 + (float)k) * f13 + f9, (f18 + (float)i2 + 0.5F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + 0.0F, (double)(f4 + 0.0F) + d2, f20 + (float)i2 + 0.0F, (f17 + 0.0F) * f13 + f9, (f18 + (float)i2 + 0.5F) * f13 + f11);
                        }
                    ...
                    if(j1 <= 1)
                    {
                        ...
                        {
                            tessellator.addVertexWithUV(f19 + 0.0F, (double)(f4 + f3) - d2, (f20 + (float)j2 + 1.0F) - f16, (f17 + 0.0F) * f13 + f9, (f18 + (float)j2 + 0.5F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k, (double)(f4 + f3) - d2, (f20 + (float)j2 + 1.0F) - f16, (f17 + (float)k) * f13 + f9, (f18 + (float)j2 + 0.5F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + (float)k, (double)(f4 + 0.0F) + d2, (f20 + (float)j2 + 1.0F) - f16, (f17 + (float)k) * f13 + f9, (f18 + (float)j2 + 0.5F) * f13 + f11);
                            tessellator.addVertexWithUV(f19 + 0.0F, (double)(f4 + 0.0F) + d2, (f20 + (float)j2 + 1.0F) - f16, (f17 + 0.0F) * f13 + f9, (f18 + (float)j2 + 0.5F) * f13 + f11);
                        }
            ...
            double d3, double d4, double d5)
    {
        ...
        if(s.equals("stonebrickpoof"))
        {
            mc.effectRenderer.addEffect(new EntitySlimeFX(worldObj, d, d1, d2, B2BItems.stoneBrick));
        } else        
    ...
    public void playAuxSFX(EntityPlayer entityplayer, int i, int j, int k, int l, int i1)
    {
        ...
        case 1002: 
            worldObj.playSoundEffect(j, k, l, "random.old_bow", 1.0F, 1.2F);
            break;
        ...
        case 1003: 
            if(Math.random() < 0.5D)
            {
                worldObj.playSoundEffect((double)j + 0.5D, (double)k + 0.5D, (double)l + 0.5D, "random.old_door_open", 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
            } else
            {
                worldObj.playSoundEffect((double)j + 0.5D, (double)k + 0.5D, (double)l + 0.5D, "random.old_door_close", 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
            }
            break;
        ...
        case 3000:
        	worldObj.playSoundEffect((double)j + 0.5D, (double)k + 0.5D, (double)l + 0.5D, "random.chomp", 1.0F, 1.0F / (random.nextFloat() * 0.4F + 0.8F));
        	break;
        
        case 3001:
        	worldObj.playSoundEffect((double)j + 0.5D, (double)k + 0.5D, (double)l + 0.5D, "random.slurp", 1.0F, 1.0F / (random.nextFloat() * 0.4F + 0.8F));
        	break;
        	
        case 3002:
        	worldObj.playSoundEffect((double)j + 0.5D, (double)k + 0.5D, (double)l + 0.5D, "random.old_bow", 0.5F, 0.4F / (random.nextFloat() * 0.4F + 0.8F));
        	break;
        	
        case 3003:
        	worldObj.playSoundEffect((double)j + 0.5D, (double)k + 0.5D, (double)l + 0.5D,"random.old_bow", 1.0F, 1.0F / (random.nextFloat() * 0.4F + 0.8F));
        	break;
        }
    }
