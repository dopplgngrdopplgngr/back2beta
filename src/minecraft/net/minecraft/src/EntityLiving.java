...
public abstract class EntityLiving extends Entity
{
    ...
    public void onEntityUpdate()
    {
        ...
        {
            attackEntityFrom(Damage.block, 1);
        }
        ...
        if(isEntityAlive() && isInsideOfMaterial(Material.water) && !canBreatheUnderwater())
        {
            ...
            if(air == -20)
            {
                ...
                attackEntityFrom(Damage.drown, 2);
            }
            fire = 0;
        } else
        {
    ...
    public boolean attackEntityFrom(Damage damage, int i)
    {
        ...
        if((float)heartsLife > (float)heartsHalvesLife / 2.0F)
        {
            ...
            damageEntity(damage.doesBypassArmor, i - field_9346_af);
            field_9346_af = i;
            flag = false;
        } else
        {
            ...
            damageEntity(damage.doesBypassArmor, i);
    ...
    protected void fall(float f)
    {
        ...
        if(i > 0)
        {
            attackEntityFrom(Damage.fall, i);
    ...
    protected void kill()
    {
        attackEntityFrom(Damage.generic, 4);
    }
    ...
    public void handleHealthUpdate(byte byte0)
    {
        if(byte0 == 2)
        {
            ...
            attackedAtYaw = 0.0F;
            worldObj.playSoundAtEntity(this, getHurtSound(), getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
            attackEntityFrom(Damage.generic, 0);
        } else
        if(byte0 == 3)
        {
            worldObj.playSoundAtEntity(this, getDeathSound(), getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
            health = 0;
            onDeath(Damage.generic);
        } else
        {
