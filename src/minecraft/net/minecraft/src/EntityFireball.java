...
public class EntityFireball extends Entity
{
    ...
    public void onUpdate()
    {
        ...
        for(int j = 0; j < list.size(); j++)
        {
            ...
            if(!entity1.canBeCollidedWith() || entity1 == owner && field_9395_l < 25)
            {
                continue;
            }
            float f2 = 0.3F;
        ...
        if(movingobjectposition != null)
        {
            if(!worldObj.multiplayerWorld)
            {
                ...
                {
                    if(!movingobjectposition.entityHit.attackEntityFrom(Damage.mobIndirect(owner, this), 0));
                }
                worldObj.newExplosion(Damage.explode(this), posX, posY, posZ, 1.0F, true);
            }
            setEntityDead();
        }
        posX += motionX;
