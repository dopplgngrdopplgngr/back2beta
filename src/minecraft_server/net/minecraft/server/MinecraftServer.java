    ...
    implements Runnable, ICommandListener
{
        ...
        throws UnknownHostException
    {
        ...
        logger.info((new StringBuilder()).append("Starting minecraft server version ").append(B2B.version).toString());
    ...
    private void initWorld(ISaveFormat isaveformat, String s, long l)
    {
        ...
        for(int i = 0; i < worldMngr.length; i++)
        {
            if(i == 0)
            {
                worldMngr[i] = new WorldServer(this, saveolddir, s, i == 0 ? 0 : -1, l);
            } else
            {
                worldMngr[i] = new WorldServerMulti(this, saveolddir, s, i == 0 ? 0 : -1, l, worldMngr[0]);
            }
            worldMngr[i].addWorldAccess(new WorldManager(this, worldMngr[i]));
            worldMngr[i].difficultySetting = propertyManagerObj.getIntProperty("difficulty", 2);
