...
public class EntityWolf extends EntityAnimal
{
    ...
    protected String getLivingSound()
    {
        ...
        if(rand.nextInt(3) == 0)
        {
            if(isWolfTamed() && dataWatcher.getWatchableObjectInteger(18) < 10)
            {
    ...
    protected void updatePlayerActionState()
    {
        ...
        if(!hasAttacked && !getGotPath() && isWolfTamed() && ridingEntity == null)
        {
        ...
        if(playerToAttack == null && !getGotPath() && !isWolfTamed() && worldObj.rand.nextInt(100) == 0)
        {
        ...
        if(func_25021_O() && !getGotPath() && !getIsAngry())
        {
            ...
            if(entity instanceof EntityPlayer)
            {
                ...
                if(itemstack != null)
                {
                    ...
                    {
                        field_25039_a = ((ItemFood)Item.itemsList[itemstack.itemID]).isFoodWolfFavorite();
                    }
                }
            }
        }
    ...
    public boolean attackEntityFrom(Damage damage, int i)
    {
        ...
        if(super.attackEntityFrom(damage, i))
        {
            if(!isWolfTamed() && !getIsAngry())
            {
                if(entity instanceof EntityPlayer)
                {
                    setIsAngry(true);
            ...
            {
                if(isWolfTamed() && (entity instanceof EntityPlayer) && ((EntityPlayer)entity).username.equalsIgnoreCase(getOwner()))
                {
                    return true;
                }
                playerToAttack = entity;
    ...
    protected void attackEntity(Entity entity, float f)
    {
        ...
        if((double)f < 1.5D && entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY)
        {
            ...
            entity.attackEntityFrom(Damage.mob(this), byte0);
        }
    }
    ...
    public boolean interact(EntityPlayer entityplayer)
    {
        ...
        } else
        {
            if(itemstack != null && (Item.itemsList[itemstack.itemID] instanceof ItemFood))
            {
                ...
                if(itemfood.isFoodWolfFavorite() && dataWatcher.getWatchableObjectInteger(18) < 20)
                {
                    itemstack.stackSize--;
                    if(itemstack.stackSize <= 0)
                    {
