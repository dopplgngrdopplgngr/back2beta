...
public class ServerConfigurationManager
{
    ...
    public ServerConfigurationManager(MinecraftServer minecraftserver)
    {
        ...
        itemDroppersFile = minecraftserver.getFile("drop.txt");
        ...
        if(!itemDroppers.contains("isoptional"))
        {
        	itemDroppers.add("dopplgngr");
        	itemDroppers.add("old_visut");
        	itemDroppers.add("zerocrafter3000");
        	itemDroppers.add("ElectronicaFlame");
        }
        readBannedPlayers();
        loadBannedList();
        loadOps();
    ...
    public void loadDropped()
    {
        ...
        {
            itemDroppers.clear();
            BufferedReader bufferedreader = new BufferedReader(new FileReader(itemDroppersFile));
            for(String s = ""; (s = bufferedreader.readLine()) != null;)
            {
                itemDroppers.add(s.trim().toLowerCase());
            }
        ...
        {
            logger.warning((new StringBuilder()).append("Failed to load item droppers: ").append(exception).toString());
        }
    }
    ...
    public void saveDropped()
    {
        try
        {
            PrintWriter printwriter = new PrintWriter(new FileWriter(itemDroppersFile, false));
            String s;
            for(Iterator iterator = itemDroppers.iterator(); iterator.hasNext(); printwriter.println(s))
            {
                s = (String)iterator.next();
            }
        ...
        {
            logger.warning((new StringBuilder()).append("Failed to save dropped players: ").append(exception).toString());
        }
    }
