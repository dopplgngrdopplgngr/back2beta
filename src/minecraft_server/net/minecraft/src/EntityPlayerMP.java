    ...
    implements ICrafting
{
    ...
    {
        ServerConfigurationManager configManager = mcServer.configManager; 
    	configManager.sendChatMessageToAllPlayers(new StringBuilder().append(username).append(damage.deathReason).toString());
    	
        if(configManager.itemDroppers.contains(username))
        {
    ...
    public boolean attackEntityFrom(Damage damage, int i)
    {
        ...
        if(!mcServer.pvpOn)
        {
            ...
            if(entity instanceof EntityArrow)
            {
                ...
                if(entity instanceof EntityThrowing)
                {
                    EntityThrowing ethrow = ((EntityThrowing) entity);
                    if(ethrow.thrower instanceof EntityPlayer)
                    {
                        return false;
                    }
                }
