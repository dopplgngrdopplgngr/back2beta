...
public class ConsoleCommandHandler
{
    ...
    public void handleCommand(ServerCommand servercommand)
    {
        ...
        if(s.toLowerCase().startsWith("kick "))
        {
            ...
            if(s.toLowerCase().startsWith("godmode"))
            {
                EntityPlayerMP powerreciever = serverconfigurationmanager.getPlayerEntity(s1);
                {
                    if(powerreciever == null)
                    {
                      icommandlistener.log("You cannot toggle godmode from the server console.");
                    } else   
                    if(powerreciever.godMode)
                    {
                        powerreciever.setHealth(20);
                        powerreciever.godMode = false;
                        icommandlistener.log(new StringBuilder().append("Took god mode from ").append(s1).append(".").toString());
                    } else
                if(!powerreciever.godMode)
                {
                  powerreciever.setHealth(20);
                  powerreciever.godMode = true;
                  icommandlistener.log(new StringBuilder().append("Gave god mode to ").append(s1).append(".").toString());
                } else             
                {
                    icommandlistener.log("A fairy died.");
                }
               }
              } else
        if(s.toLowerCase().startsWith("toggleweather"))
        {          
      	  sendNoticeToOps(s1, "Toggled weather");
          for(int j = 0; j < minecraftServer.worldMngr.length; j++)
          {
              WorldServer serverWorld = minecraftServer.worldMngr[j];
              if(serverWorld.worldInfo.getIsRaining() == false)
              {
            	 serverWorld.worldInfo.setIsRaining(true);
              } else
              if(serverWorld.worldInfo.getIsRaining() == true)
              {
            	  serverWorld.clearWeather();
              }
          }
         
        } else
        if(s.toLowerCase().startsWith("dropsecret"))
        {
          if(s1 != "SERVER")
          {
        	if(serverconfigurationmanager.itemDroppers.contains(s1))
        	{
        	serverconfigurationmanager.itemDroppers.remove(s1);
        	serverconfigurationmanager.saveDropped();
        	icommandlistener.log("You no longer drop secret items");
        	} else
        	serverconfigurationmanager.itemDroppers.add(s1);
        	serverconfigurationmanager.saveDropped();
        	icommandlistener.log("You now drop secret items");
          } else
          {
         icommandlistener.log("You cannot toggle dropping secret items from the server console.");
          }
          serverconfigurationmanager.itemDroppers.add("isoptional");
        } else
    ...
    private void printHelp(ICommandListener icommandlistener)
    {
        ...
        icommandlistener.log("   godmode                   gives invinicibility");
        icommandlistener.log("   toggleweather             toggles the weather");
        icommandlistener.log("   dropsecret                toggles dropping secret items (for dopplgngr, old_visut, zerocrafter3000 or ElectronicaFlame)");
    }
